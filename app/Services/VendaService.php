...
class VendaService implements VendaServiceInterface
{
    ...
    public function salvaVenda($dadosVenda)
    {
        ...
        for ($i=0; $i < (int)$dadosVenda->crediario_loja_parcela; $i++) {
            ...
            $dadosParcelamento[$i]['id_venda'] = $venda;
        ...
        foreach ($produtosVenda as $key => $value) {
            foreach ($vendaProdutosAgrupados as $k => $v) {
                if($v[0]['id_prod'] == $value['id_prod']) {
                    $produtosFormatado[$key]['id_venda'] = $venda;
    ...
    {
        return $this->repository->deletaVenda($idVenda);
    }

    public function totaisVendas($uuid)
    {
        $totais = $this->repository->totaisVendas($uuid);
        $totaisFormatado = [];
        
        foreach ($totais as $key => $value) {
            switch ($value->payment_type) {
                case 'billet':
                    $totaisFormatado[$key]['forma'] = 'Vendas Boleto';
                    break;
                case 'credit_card':
                    $totaisFormatado[$key]['forma'] = 'Vendas Cartão de Crédito';
                    break;
                case 'PIX':
                    $totaisFormatado[$key]['forma'] = 'Vendas no Pix';
                    break;
                case 'WALLET':
                    $totaisFormatado[$key]['forma'] = 'Wallet';
                    break;
                case 'paypal':
                    $totaisFormatado[$key]['forma'] = 'PayPal';
                    break;
            }
            $totaisFormatado[$key]['total_vendido'] = Utils::formataPreco($value->total);
        }

        return $totaisFormatado;
    }
}